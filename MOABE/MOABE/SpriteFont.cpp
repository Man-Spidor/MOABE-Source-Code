	...
	FontTexture(nullptr)
{
	...
	std::wstring texturePath = w_path + L".tga";

	this->FontTexture = new Texture(texturePath.c_str(), SamplerManager::GetDefault());

	// Parse associated XML file
	XMLtoCollection(path + ".xml");

	this->tabSize = 2.0f * this->GetGlyph('A')->GetWidth();
	this->newLineSize = this->GetGlyph('A')->GetHeight();
}

SpriteFont::~SpriteFont()
{
	for (auto g : this->fontmap) {
		delete g.second;
	}
	...
	while (S_OK == reader->Read(&nodeType))
	{
		...
		case XmlNodeType::XmlNodeType_EndElement:
		{
			...
			if (wcscmp(stringValue, L"character") == 0)
			{
				...
				std::string imageKey = this->Name + std::to_string(key);
				ImageManager::Load(imageKey.c_str(), this->FontTexture, new jAzulRect((float)x, (float)y, (float)w, (float)h));

				Glyph* pGlyph = new Glyph(ImageManager::Get(imageKey.c_str()));
				pGlyph->SetCenter((float)-w, (float)h);

				this->fontmap[key] = pGlyph;
...
const float SpriteFont::GetTabSize() const
{
	return this->tabSize;
}

const float SpriteFont::GetNewLineSize() const
{
